...
- ### Creating a hash or link of any chat which has all its context and data of the conversation and we can import all that info using the link or hash to any other chat for
  
- ### An AI chatbot/api which takes programmed prompts like for example: I need to make a list of all the bike prices of royal enfield having 200-300 cc and compare it with other bike companies on the same engine range so my prompt will be "((create a list of all bikes having 200-300cc engine) compare this with (list of all royal enfield bikes with 200-300cc))"                        So as here are 3 () braces in total hence there will be 3 chat instance of ai will be running. Each working to provide the best result for their specified task
  
- ### An extension that reads the full page that user currently has focus on and understands the context then highlights important phrases by changing the bg color of text in the browser's javascript or by any other means. just like we highlight texts in a book. It can have three levels of highlighting Concise, moderate or detailed. We can also add a feature which summarizes the selected text and displays the summary in a floating component or a dialog box just like in cursor or antigravity we add the selected text of a code file into the chat window the user can select a part of text then add it to the floating point and ask anything from it "summarize, explain in detail, show some examples with links, etc"