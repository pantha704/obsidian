...
// src/main.ts
__export(exports, {
  default: () => BetterInlineFieldsPlugin
});
...
function getIndicesOf(text, search) {
  const indices = [];
  if (!text)
    return indices;
  let startIndex = 0;
...
var BetterInlineFieldsSettingTab = class extends import_obsidian3.PluginSettingTab {
  ...
  display() {
    ...
    new import_obsidian3.Setting(this.containerEl).setName("Add New").setDesc("Add new autocomplete field").addButton((button) => {
...
// src/suggestions/PagesEditSuggest.ts
...
var PagesEditSuggest = class extends import_obsidian4.EditorSuggest {
  ...
  getSuggestions(context) {
    const api = (0, import_obsidian_dataview.getAPI)(this.app);
    ...
    const searchFolder = this.plugin.settings.autocomplete.find((autocomplete) => autocomplete.field === field);
    if (!searchFolder)
      return [];
  ...
  selectSuggestion(suggestion, event) {
    ...
    if (!activeView || !startPos || !endPos)
      return;
    let text = suggestion.label;
    if (suggestion.isEmptyChoice && suggestion.field) {
      text = suggestion.query;
      const folderPath = this.plugin.settings.autocomplete.find((autocomplete) => autocomplete.field === suggestion.field)?.folder;
      if (folderPath) {
